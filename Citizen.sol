...
contract Citizen {
    ...
    string
        public help_gestion_status = "change le statut 0=travailleur, 1=chomeur, 2=malade, 3=mort";
    ...
    modifier onlySage() {
        ...
        require(
            admin[msg.sender].delayRegistration > block.timestamp,
            "vous n'êtes plus sage"
        );
        _;
    }
    ...
    modifier onlyCitoyen() {
        require(
            administration[msg.sender].citoyen == true,
            "fonction réservée aux citoyens"
        );
        require(administration[msg.sender].banni < block.timestamp);
        _;
    }
    ...
    function register(address _addr, uint256 _age) public onlySage {
        require(
            administration[_addr].citoyen == false,
            "cette personne est déjà citoyen"
        );
    ...
    function gestion_statut(statut _ide, address _addr) public onlySage {
        if (_ide == statut.travailleur) {
            administration[_addr].travailleur
                ? administration[_addr].travailleur = false
                : (administration[_addr].travailleur = true) &&
                    (administration[_addr].chomeur = false) &&
                    (administration[_addr].malade = false) &&
                    (administration[_addr].retraite = false);
        } else if (_ide == statut.chomeur) {
            administration[_addr].chomeur
                ? administration[_addr].chomeur = false
                : administration[_addr].chomeur =
                true &&
                (administration[_addr].travailleur = false) &&
    ...
    {
        require(
            administration[msg.sender].wallet >= 100,
            "vous ne possédez pas la somme a mettre en dépot pour vous présenter"
        );
    ...
    function vote(uint256 _nbvote, choix bulletin) public onlyCitoyen {
        //    require ((block.timestamp)<(a_voter[_nbvote].temps_vote), "ce vote est terminé");
        require(
            a_voter[_nbvote].didVote[msg.sender] == false,
            "vous avez déjà voté"
        );
        if (bulletin == choix.Oui) {
            ...
            revert("choix pour le vote incompris");
        }
    }

    function result(uint256 _nbvote)
        public
        view
        returns (
            string memory,
        ...
        )
    {
        ...
        if (a_voter[_nbvote].voteOui > a_voter[_nbvote].voteNon) {
            return (
                ...
                a_voter[_nbvote].voteOui,
                a_voter[_nbvote].voteNon
            );
        } else {
            return (
    ...
    function gestionSage(uint256 _nbvote) public onlySage {
        ...
        admin[(a_voter[_nbvote].candidat)].delayRegistration =
            block.timestamp +
            8 weeks;
    }

    enum tribunal {innocent, legere, lourde, grave, crime}

    function jugerSoumettre(address _condamne)
        public
        onlySage
        returns (
            string memory,
    ...
    {
        require(
            administration[(a_voter[compteur].candidat)].citoyen == true,
            "on ne juge que les citoyens"
        );
        compteur += 1;
        a_voter[compteur].id = compteur;
        a_voter[compteur].candidat = _condamne;
        a_voter[compteur].temps_vote = block.timestamp + 2 minutes;
        return ("le vote", a_voter[compteur].id, "est en cours");
    }

    function jugerVote(uint256 _nbvote, tribunal bulletin) public onlyCitoyen {
        //    require ((block.timestamp)<(a_voter[_nbvote].temps_vote), "ce vote est terminé");
        require(
            a_voter[_nbvote].didVote[msg.sender] == false,
            "vous avez déjà voté"
        ...
        require(
            a_voter[_nbvote].candidat != msg.sender,
            "le jugé ne peut pas voter"
        );

        if (bulletin == tribunal.innocent) {
            a_voter[_nbvote].innocent += 1;
            a_voter[_nbvote].didVote[msg.sender] = true;
        } else if (bulletin == tribunal.legere) {
            a_voter[_nbvote].legere += 1;
            a_voter[_nbvote].didVote[msg.sender] = true;
        } else if (bulletin == tribunal.lourde) {
            a_voter[_nbvote].lourde += 1;
            a_voter[_nbvote].didVote[msg.sender] = true;
        } else if (bulletin == tribunal.grave) {
            a_voter[_nbvote].grave += 1;
            a_voter[_nbvote].didVote[msg.sender] = true;
        } else if (bulletin == tribunal.crime) {
            a_voter[_nbvote].crime += 1;
            a_voter[_nbvote].didVote[msg.sender] = true;
        } else {
            revert("choix pour le vote incompris");
        }
